...
RATE_LIMIT_DELAY = float(os.environ.get('RATE_LIMIT_DELAY', '0.05'))  # Reduced from 0.1
...
def process_single_file(service, file, owner_email, backup_date, drive_name=None):
...
def process_files_batch(service, files, owner_email, backup_date, drive_name=None):
            ...
            for owner_email, files in files_by_owner.items():
                ...
                for i in range(0, len(files), BATCH_SIZE):
                    ...
                    success, failed, bytes_transferred = process_files_batch(
                        service, batch, owner_email, backup_date
                    )
            ...
            for drive in shared_drives:
                ...
                if drive_files:
                    ...
                    for i in range(0, len(drive_files), BATCH_SIZE):
                        ...
                        success, failed, bytes_transferred = process_files_batch(
                            service, batch, 'shared-drive', backup_date, drive_name
                        )
